...
window.onscroll = function() {BarraDeProgreso()};
  ...
  $.ajax({
    ...
    success: function(data)
    {
      if (data['alerta']==='Si')
      {
        ...
        var curp2 = data['curp2']
        var domicilio = data['domicilio']
        var date = data['date']
        var location = data['location']
        var filename = data['filename']
        var filename2 = data['filename2']
        var filename3 = data['filename3']
        var correo = data['correo']
        var evento = data['evento']
        console.log(nombre,nombre2,curp1,curp2,domicilio,date,location,filename,filename2,filename3,correo,evento)
        $(".nombre").html(nombre);
        ...
        document.getElementById("ine-2").src=`http://127.0.0.1:5000/static/upload/img/${filename2}`;
